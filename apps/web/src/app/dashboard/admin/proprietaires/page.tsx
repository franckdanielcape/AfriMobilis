...
function ProprietairesGlobauxContent() {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const response = await fetch('/api/users?role=proprietaire');
                ...
                console.error('Erreur fetch proprietaires:', error);
                setUsers([]);
            } finally {
                setLoading(false);
            }
        ...
        <div style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
                ...
                <div>
                    ...
                    <p style={{ color: 'var(--text-secondary)', margin: 0 }}>Gestion et vue d&apos;ensemble des propriétaires de la plateforme</p>
                </div>
            </div>
            ...
            <div style={{ background: 'var(--panel-bg)', borderRadius: '12px', border: '1px solid var(--border)', padding: '1.5rem', marginBottom: '2rem' }} className="glass-panel">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                    ...
                    <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
                        <strong>{filteredUsers.length}</strong> propriétaire(s) trouvé(s)
                    </div>
                </div>

                {loading ? (
                    <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-secondary)' }}>Chargement des propriétaires...</div>
                ) : (
...
export default function ProprietairesGlobauxPage() {
    return (
        <Suspense fallback={<div style={{ padding: '2rem', textAlign: 'center' }}>Chargement...</div>}>
            <ProprietairesGlobauxContent />
        </Suspense>
    );
}
