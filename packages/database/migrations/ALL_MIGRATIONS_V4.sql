-- =====================================================
-- MIGRATIONS V4 - CORRECTION DÉFINITIVE
-- =====================================================

-- =====================================================
-- 0. RÉPARER LA TABLE TICKETS (CORRECTION COLONNES)
-- =====================================================

-- Voir quelles colonnes existent dans tickets
DO $$
DECLARE
    v_has_created_at BOOLEAN;
    v_has_date_creation BOOLEAN;
    v_has_updated_at BOOLEAN;
BEGIN
    SELECT EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name = 'tickets' AND column_name = 'created_at') INTO v_has_created_at;
    SELECT EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name = 'tickets' AND column_name = 'date_creation') INTO v_has_date_creation;
    SELECT EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name = 'tickets' AND column_name = 'updated_at') INTO v_has_updated_at;
    
    RAISE NOTICE 'Colonnes tickets: created_at=%, date_creation=%, updated_at=%', v_has_created_at, v_has_date_creation, v_has_updated_at;
END $$;

-- Ajouter created_at si aucune colonne de date n'existe
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tickets' AND column_name = 'created_at')
       AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tickets' AND column_name = 'date_creation')
       AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tickets' AND column_name = 'updated_at') THEN
...
-- =====================================================
-- CRÉATION TABLES DE BASE (SI MANQUANTES)
-- =====================================================
...
-- =====================================================
-- MIGRATION 011: TICKETS ET COMMENTAIRES
-- =====================================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tickets' AND column_name = 'traite_par') THEN
        ALTER TABLE tickets ADD COLUMN traite_par UUID REFERENCES profiles(id);
    END IF;
...
-- Vue tickets - UTILISER UNIQUEMENT created_at
...
CREATE TABLE objets_perdus (
    ...
    statut VARCHAR(50) DEFAULT 'en_attente',
    objet_trouve_id UUID REFERENCES objets_trouves(id),
    date_matching TIMESTAMP,
    matched_by UUID REFERENCES profiles(id),
    date_rendu TIMESTAMP,
    rendu_a UUID REFERENCES profiles(id),
    notes_rendu TEXT,
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW()
);
...
BEGIN
    ...
    PERFORM creer_notification(v_passager_id, 'system', 'info', 'Objet rendu',
        'Votre objet vous a été rendu.', '/dashboard/tickets', 'objet_perdu', p_objet_perdu_id);
...
BEGIN
    ...
    RETURN QUERY
    SELECT pa.type_panne, COUNT(*) as nombre, COALESCE(SUM(pa.cout_reparation), 0) as cout_total
    FROM pannes pa
    JOIN vehicules ve ON ve.id = pa.vehicule_id
    WHERE ve.proprietaire_id = p_proprietaire_id AND pa.date_declaration BETWEEN p_date_debut AND p_date_fin
...
SELECT '=== MIGRATIONS V4 TERMINÉES ===' as status;

SELECT 'Tables créées:' as info;
SELECT table_name FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('notifications', 'ticket_comments', 'objets_perdus', 'objets_trouves', 'documents', 'versements', 'pannes', 'vehicules', 'profiles')
ORDER BY table_name;
